...
interface EditPlaylistDialogProps {
  ...
  onUpdate: (playlistId: string, title: string, isPublic: boolean) => Promise<void>;
}

export function EditPlaylistDialog({ isOpen, onClose, playlist, onUpdate }: EditPlaylistDialogProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await onUpdate(playlist.id, title.trim(), isPublic);
      onClose();
    } catch (err) {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm">
      <div className="bg-[#1a1a1a] rounded-2xl p-6 w-full max-w-md border border-[#2a2a2a] shadow-2xl">
        <div className="flex items-center justify-between mb-6">
          <h2 className="text-white text-2xl font-bold">Edit Playlist</h2>
          <button
            onClick={handleClose}
            disabled={isLoading}
